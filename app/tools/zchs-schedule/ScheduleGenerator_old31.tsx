...
type Preset = 'today' | 'this-week' | 'next-week' | 'custom'
...
function getPresetTooltip(preset: Preset): string | undefined {
  ...
  if (dates.end < SCHOOL_YEAR_START) {
    if (preset === 'today') return 'Available starting August 4, 2026'
...
export default function ScheduleGenerator() {
  ...
  const [collapsedS1, setCollapsedS1] = useState(false)
  const [collapsedS2, setCollapsedS2] = useState(false)
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 print:py-0 print:px-0 print:max-w-none">
      ...
      <div className="print:hidden">
        ...
        {/* Student + date range */}
        <section className="mb-8 p-6 bg-stone-50 border border-stone-200 rounded-lg max-w-[720px]">
            ...
            {preset === 'custom' ? (
              ...
              <p className="text-sm text-stone-600">
                {startDate && endDate
                  ...
                  : 'Choose a preset to set the date range.'}
              </p>
            )}
          </div>
        </section>
        ...
        {/* Short print behavior note — screen only, small stone-500. */}
        <p className="text-sm text-stone-500 mb-6 max-w-[720px] print:hidden">
          Print is condensed. Screen preview shows all details.
        </p>

        {validationError && (
          <div className="mb-6 p-4 bg-brick-50 border border-brick-200 text-brick-900 rounded-md">
            {validationError}
          </div>
        )}
      </div>
    ...
    <section className="mb-8 max-w-[720px] print:hidden">
      <div className="mb-2 flex items-baseline gap-3 flex-wrap">
        ...
        <span className="text-sm text-stone-500">
          Optional. Add clubs, sports, rehearsals, etc.
        </span>
      </div>

      <p className="text-sm text-stone-600 mb-4">
        Print shows up to {CAP_BEFORE} before-school and {CAP_AFTER}{' '}
        after-school activities per day.
      </p>
          ...
          {(
            [
              ...
              },
              {
                ...
                example:
                  'e.g., private lessons, community organizations, performances, competitions',
              },
            ] as const
          ).map((opt) => {
            const selected = activity.schoolDaysOnly === opt.value
            return (
              <div key={String(opt.value)} className="flex-1 min-w-[200px]">
                <button
                  type="button"
    ...
    if (activityAppliesToDateRaw(proposed, dateKey)) {
      ...
      if (isWeekend) {
        ...
        if (count > CAP_WEEKEND) {
          return {
            valid: false,
            message: `This activity won't fit on the printed calendar on some dates. Print shows up to ${CAP_BEFORE} before-school and ${CAP_AFTER} after-school activities per day (${CAP_WEEKEND} on weekends).`,
          }
        }
      } else {
        ...
        if (count > cap) {
          return {
            valid: false,
            message: `This activity won't fit on the printed calendar on some dates. Print shows up to ${CAP_BEFORE} before-school and ${CAP_AFTER} after-school activities per day.`,
          }
        }
      }
    }
              ...
              return (
                ...
                >
                  {day ? (
                    ...
                    <div className="text-sm print:text-xs font-medium text-stone-500">
                      {weekday}
                    </div>
                  )}
                </th>
                    ...
                    {list.length > 0 && (
                      <ul className="space-y-0.5 print:space-y-0 text-left print:leading-tight">
                        {list.map((a, idx) => (
                          <li
                            key={a.id}
                            className={`text-[11px] print:text-[10px] text-stone-800 leading-tight break-words print:line-clamp-1 ${
            ...
            return (
              <tr key={periodNum} className="h-24 print:h-14">
                ...
                {week.days.map((day, slot) => {
                  ...
                  // Blocked (no-school) day: merge all 4 period rows into one cell
                  // showing the reason. Only render on the first period row.
                  if (day.isBlocked) {
              ...
              {(
                ...
                <Fragment key={weekday}>
                  <td className="p-1.5 print:p-1 align-middle text-center border border-stone-300 bg-stone-50">
                    <div className="text-xs print:text-[10px] font-semibold text-stone-900 uppercase tracking-wide whitespace-nowrap">
