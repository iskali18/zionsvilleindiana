...
export default function ScheduleGenerator() {
  ...
  /** Precomputed "over-limit" banner JSX (or null). Only recomputes when the
   *  active student's activities list changes — not on every keystroke or
   *  unrelated state change. */
  const wontFitBannerContent = useMemo(() => {
          ...
          __html: `
            @media print {
              @page {
                ...
                @bottom-center {
                  content: "Printed ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} · ZionsvilleIndiana.com/tools/zchs-schedule";
...
interface WeekGridProps {
  week: WeekBucket
  activities: Activity[]
  /** Print range boundaries — used to gate weekend Sat/Sun rendering when
   *  the range starts or ends mid-week. */
  printStart: string
  printEnd: string
}

function WeekGrid({ week, activities, printStart, printEnd }: WeekGridProps) {
  ...
  // Only render each day if it falls within the print range.
  const satKey = toDateKey(addDays(makeDate(week.weekStart), 5))
  const sunKey = toDateKey(addDays(makeDate(week.weekStart), 6))
  const satInRange = satKey >= printStart && satKey <= printEnd
  const sunInRange = sunKey >= printStart && sunKey <= printEnd
...
function WeekStacked({ week, activities, printStart, printEnd }: WeekGridProps) {
  ...
  // the week has any Sat/Sun activities AND the day falls within print range.
  const satKey = toDateKey(addDays(makeDate(week.weekStart), 5))
  const sunKey = toDateKey(addDays(makeDate(week.weekStart), 6))
  const satInRange = satKey >= printStart && satKey <= printEnd
  const sunInRange = sunKey >= printStart && sunKey <= printEnd
